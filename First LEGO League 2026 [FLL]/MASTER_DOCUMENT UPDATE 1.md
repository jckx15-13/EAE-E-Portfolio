...
**Version:** v51.0.0 | **Last Updated:** March 2026 | **Status:** Active Development
...
|----|------|--------|-------------|
| CRASH-001 | path_simulation.py | ✅ FIXED v49 | RuntimeStatisticsCollector.timed() missing |
| CRASH-002 | spike_code_merger.py | ✅ FIXED v48 | File replaced with tactical_hud.py copy |
| CRASH-003 | tactical_hud.py | ✅ FIXED v50.1 | ToolTip.show() uses winfo_rootx/y in try/except TclError |
| CRASH-004 | mission_editor.py | ✅ FIXED v47.2 | CalibrationTab reference removed |
| BUG-001 | path_simulation.py | ✅ FIXED v49 | format_duration alias added |
| BUG-002 | diagnostic_dashboard.py | ✅ FIXED v47.1 | dx/dy stale on final graph edge |
| BUG-003 | mission_editor.py | ✅ FIXED v49.1 | pack/grid mix removed |
| BUG-004 | mission_editor.py | ✅ FIXED v49.1 | Name Entry validate= added |
| BUG-005 | Fll_command_engine.py | ✅ FIXED v50.1 | View switching now uses lift() — window stable |
| BUG-006 | tactical_hud.py | ✅ FIXED v48 | _build_menu_bar() called from _build_ui() |
| BUG-007 | diagnostic_dashboard.py | ✅ FIXED v47.1 | _compute_state_hash includes m.name + m.time_sec |
| BUG-LAUNCH-001 | launcher.py | ✅ FIXED v50.1 | logging.getlogging → getLogger; self.logging → self.logger |
| BUG-ENG-001 | fll_engine.py | ✅ FIXED v3.1 | PracticeTracker.log_run → add_run(RunResult) |
| BUG-ME-001 | mission_editor.py | ✅ FIXED v49.1 | matplotlib.pyplot in destroy → fig.clf() |
| BUG-ME-002 | mission_editor.py | ✅ FIXED v49.1 | TestMissionEditorUI missing setUp tk.Tk() root |
| BUG-DIAG-001 | diagnostic_dashboard.py | ✅ FIXED v50.1 | SparklineGraph.add_point try/except TclError |
| BUG-PATH-001 | path_simulation.py | ✅ FIXED v50.1 | from __future__ moved to line 1 |
| BUG-PATH-002 | path_simulation.py | ✅ FIXED v50.1 | RectangleObstacle.collides_with_segment wrong algorithm |
| BUG-CMD-001 | Fll_command_engine.py | ✅ FIXED v50.1 | _build_top_bar bare legacy → self._legacy |
| BUG-HUD-001 | tactical_hud.py | ✅ FIXED v50.1 | matplotlib.pyplot in destroy → fig.clf() |
| DATA-001 | path_simulation.py | ✅ FIXED v50.2 | 14/15 MISSION_DEFS coords wrong vs official xlsx |
| ARCH-001 | spike_code_merger.py | ✅ FIXED v48 | 13 duplicate class definitions |
| ARCH-004 | All files | ✅ FIXED v50 | Changelog string literals → # comments (all files) |
| ORPHAN-001 | config.py | ✅ FIXED v50 | Now imported by fll_engine.py + Fll_command_engine.py |
| STYLE-001 | tactical_hud.py | ⚠ OPEN | ToolTip <Destroy> binding missing — possible orphaned windows |
| STYLE-003 | launcher.py | ✅ FIXED v49 | assert hasattr() → RuntimeError |
| STYLE-006 | diagnostic_dashboard.py | ✅ FIXED v47.1 | Windows-only fonts replaced |
| PERF-001 | launcher.py | ✅ FIXED v47 | time.sleep on main thread |
| PERF-002 | launcher.py | ✅ FIXED v47 | Flat 16ms polling |

---

## 12. CHANGELOG

### v51.0.0 (Current — March 2026) — Recursive Audit + Data Corrections
**Bug fixes (7 crashes/runtime + 2 algorithm + 14 data):**
- launcher.py v50.1.0: `logging.getlogging` typo fixed (4 sites); `self.logging` → `self.logger` (26 sites, was shadowing stdlib); type hints corrected
- fll_engine.py v3.1.0: `PracticeTracker.log_run` → `add_run(RunResult)` — method did not exist
- mission_editor.py v49.1.0: `matplotlib.pyplot` in `destroy()` → `fig.clf()`; TestMissionEditorUI `setUp`/`tearDown` with `tk.Tk()` root added; `bare except` → `except OSError`
- diagnostic_dashboard.py v50.1.0: `SparklineGraph.add_point` wrapped in `try/except tk.TclError` — tests crashed after `root.destroy()`
- path_simulation.py v50.1.0: `from __future__` moved to line 1; `RectangleObstacle.collides_with_segment` fixed — was checking only the start-point of the path against each obstacle edge (used `point_line_distance(start, edge)` instead of `segment_segment_distance(path, edge)`); new `segment_segment_distance()` helper added
- path_simulation.py v50.2.0: **14/15 MISSION_DEFS coordinates corrected** against official `FLL_Mission_Data.xlsx` and wireframe grid. Errors ranged from 15cm (M13) to 186cm (M15); M07 and M11 were 111cm off. Routing quality was significantly degraded.
- Fll_command_engine.py v50.1.0: `_build_top_bar()` referenced bare `legacy` parameter out of scope; fixed to `self._legacy`
- tactical_hud.py v50.1.0: `matplotlib.pyplot` in `destroy()` → `fig.clf()`

**All tests pass** (recursive self-test confirms no banned anti-patterns in any of 9 output files)

### v49.0.0 (Previous — March 2026)

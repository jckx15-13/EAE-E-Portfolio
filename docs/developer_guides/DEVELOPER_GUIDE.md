...
The timeline is intentionally designed as a central spine with alternating cards on each side, plus year filtering, so evaluators can both see the time order and compare achievements by era or category.
...
1. **Fixed Header Chrome**: Incorporates the brand signature, primary section navigation links, and a reading progress bar.
  * The progress bar is enhanced with a glowing gradient fill and smooth animation to make scroll position more visible and engaging.
2. **View Mode Toggle Bar**: A sticky control container supporting "Cards", "Timeline", and "Story" mode buttons.
  * The page also includes a floating, stationary timeline/story slider widget that appears only when the achievement timeline is in view, preserving page focus elsewhere.
